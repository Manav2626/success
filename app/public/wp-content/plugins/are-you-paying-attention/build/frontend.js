...
divsToUpdate.forEach(function (div) {
  ...
  react_dom__WEBPACK_IMPORTED_MODULE_1___default().render((0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)(Quiz, {
    ...data
  }), div); //question={data.question} -> {...data}
  div.classList.remove("paying-attention-update-me");
});
function Quiz(props) {
  const [isCorrect, setIsCorrect] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)();
  const [isCorrectDelayed, setIsCorrectDelayed] = (0,react__WEBPACK_IMPORTED_MODULE_0__.useState)();
  ...
  })), (0,react__WEBPACK_IMPORTED_MODULE_0__.createElement)("p", null, "Sorry, Wrong Answer!!")));
}

/* 
Logic of Correct Answer selection
    1. Get data in JSON format of block
    2. Set in HTML format and apply CSS (also for correct answer & wrong answer)
    3. On button click function loads(handleAnswer)
    4. If condion with correct answer
    5. Using React useState, to make value true or false, and loads each time
    6. Use turnary operation 
        if handleAnswer sets useState = True -> Visible Correct Answer CSS and disappear
        if handleAnswer sets useState = False -> Visible False Answer CSS and disappear
    7. Use useEffect to track Changes of useSet
        - use setTimeout -> if selected wrong answer, wait 2600ms and set setIsCorrect = UNDEFINED (so that for different wrong answer, React reloads the function)
    8. Set ternary in button onClick, if true, no more button click, if false || undefined, set that click function 
    9. If correct, Give Correct green animation + animation to unclicable button
        i. new useState delayed 
        ii. new useEffect condition to delay 1000ms
        iii. Before answer set condition and add svg for correct 
        iv. same but != CorrectAnswer give x svg
